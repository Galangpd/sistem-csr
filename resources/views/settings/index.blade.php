...
<div class="rounded-2xl border border-gray-300 shadow-lg bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
    ...
    <form action="{{ route('register.perusahaan') }}" method="PUT" enctype="multipart/form-data">
        @csrf
        @method('PUT')
      <div class="flex flex-col items-center gap-5 xl:flex-row xl:justify-between">
        <input type="file" id="photoInput" name="photo" accept="image/*" class="hidden" onchange="document.getElementById('photoForm');">

        <div class="relative w-32 h-32 cursor-pointer" onclick="document.getElementById('photoInput').click()">
            <img src="{{ asset($user->photo ?? 'asset/user.png') }}" alt="user"
                    id="photoForm"
        ...
        <div class="w-full text-start">
          ...
          <div class="w-full lg:w-1/2 px-2 lg:px-7 rounded-lg border border-gray-300 py-5">
            <form action="{{ route('register.perusahaan') }}" method="PUT">
                @csrf
                <div class="mb-6">
                    ...
                    value="{{ old('nama_perusahaan', $data->nama_perusahaan ?? '') }}" 
                     name="nama_perusahaan" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="PT Sentosa" required />
                </div>
                <div class="mb-6">
